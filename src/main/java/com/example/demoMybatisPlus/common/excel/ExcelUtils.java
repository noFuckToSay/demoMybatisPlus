...
public class ExcelUtils {
    ...
    private static String getCellValue(Cell cell) {
        ...
        if (cell.getCellTypeEnum() == CellType.NUMERIC) {
            // 小数转换会有浮点数精度问题，例如0.12会变成0.19999999
            double value = cell.getNumericCellValue();
            if ( value % 1 != 0) {
                // 如果是小数
                // 小数也会出现科学计数法问题，通过转换BigDecimal解决
    ...
    public static <T> void exportTemplate(HttpServletResponse response, String fileName, String sheetName,
        ...
        // 导出数据
        export(response, fileName, sheetName, sheetDataList, selectMap, textCols);
    }
    ...
    private static ExcelClassField getExcelClassField(Field field) {
        ...
        // 单元格格式
        cf.setIsTextStyle(annotation.isTextStyle());
        return cf;
    }
...
//        return null;
    ...
    private static <T> Map<Integer, List<String>> getSelectMap(Class<T> clazz){
// 获取表头字段
        List<ExcelClassField> headFieldList = getExcelClassFieldList(clazz);
        Map<Integer, List<String>> selectMap = new LinkedHashMap<>();
        for (int i = 0; i < headFieldList.size(); i++) {
    ...
    public static void export(HttpServletResponse response, String fileName, String sheetName,
                              List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap, List<Integer> textCols) {
        export(response, null, fileName, sheetName, sheetDataList, selectMap, textCols, null);
    }
    ...
    public static <T, K> void exportExcel(HttpServletResponse response, String fileName, List<T> list, Class<K> template) {
        if (list != null && !list.isEmpty()) {
            // 导出数据
            List<List<Object>> sheetDataList = getSheetData(list);
            Map<Integer, List<String>> selectMap = getSelectMap(list.get(0).getClass());
            export(response, fileName, sheetDataList, selectMap);
        } else {
            // 如果导出数据为空
            export(response, fileName, list, template);
        }
    ...
    private static void export(HttpServletResponse response, File file, String fileName, String sheetName,
                               List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        export(response, fileName, fileName, sheetDataList, selectMap, null);
    }

    private static void export(HttpServletResponse response, File file, String fileName, String sheetName,
                               List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap, List<Integer> textCols, SXSSFWorkbook styleTemplete) {
        // 整个 Excel 表格 book 对象
        SXSSFWorkbook book = new SXSSFWorkbook();

        //如果有传入自定义excel表头模板，则使用自定义表头模板
        if (null != styleTemplete) {
            book = styleTemplete;
        }

        // 每个 Sheet 页
        Sheet sheet = null;
        //如果没有传入自定义excel表头模板，新建sheet
        if (null == styleTemplete) {
            sheet = book.createSheet(sheetName);
        }
        //如果有传入自定义excel表头模板，则使用自定义表头模板的第一个sheet
        if (null != styleTemplete) {
        ...
        //如果没有自定义模板，设置表格列宽度的默认值;否则用自定义模板定义的列宽
        if (null == styleTemplete) {
            // 设置表格列宽度（默认为15个字节）
        ...
        if (textCols != null) {
            ...
            for (Integer col : textCols) {
                sheet.setDefaultColumnStyle(col, rowStyle);
            }
        }

        if (null != sheetDataList && !sheetDataList.isEmpty()) {  //判断要写入的数据列表不为空才执行写入excel内容
            // 创建合并算法数组
            int rowLength = sheetDataList.size();
            int columnLength = sheetDataList.get(0).size();
            int[][] mergeArray = new int[rowLength][columnLength];
            int rowNumber = 0;
            //如果有传入自定义excel表头模板,sheetDataList列表里把表头数据去掉，用自定义的表头,列表第一个子元素就是表头数据,写数据的时候从表头所占行的下一行开始写
            if (null != styleTemplete) {
...
//        response.addHeader("Content-Disposition", "attachment;filename=" + name);
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof String) {
            String s = o.toString();
            if (!StrUtil.isEmpty(s) && !s.startsWith("0") && isNumeric(s)) {
    ...
    public static <T, K> void export(HttpServletResponse response, String fileName, List<T> list, Class<K> template, SXSSFWorkbook styleTemplete) {
        // list 是否为空
        boolean lisIsEmpty = list == null || list.isEmpty();

        if (null == styleTemplete) {    //如果没有传入自定义模板，没有数据的情况下按照原来的逻辑导出空excel;如果传入了自定义模板，没有数据的情况下导出空模板
            // 如果模板数据为空，且导入的数据为空，则导出空文件
            if (template == null && lisIsEmpty) {
                exportEmpty(response, fileName);
                return;
        ...
        // 如果 list 数据，则导出模板数据
        if (lisIsEmpty && null != template) {
            exportTemplate(response, fileName, template);
            return;
        }
        // 导出数据
        List<List<Object>> sheetDataList = list.isEmpty() ? new ArrayList<>() : getSheetData(list);
        export(response, null, fileName, fileName, sheetDataList, null, null, styleTemplete);
    }
}
